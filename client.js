...
var FiveEye = {
  ...
  compareVector: function StandardDeviation(arr, level) {
    ...
    var denyia = arr[2] - level[2];
    return Math.sqrt(lightI * lightI + lightJ * lightJ + denyia * denyia);
  },
...
var antiNoClip = async function compare() {
  ...
  var jeneal = await FiveEye.checkConditions([LocalPlayer.state.bypassNoClip === "false", !IsEntityAttachedToAnyPed(keywords), !IsEntityAttachedToAnyVehicle(keywords), !IsCutscenePlaying(), !IsPlayerTeleportActive(), !IsEntityDead(keywords), !IsPedInAnyVehicle(keywords, true), !IsPedOnVehicle(keywords), !IsPedJumpingOutOfVehicle(keywords), GetPedParachuteState(keywords) <= 0, !IsPedInParachuteFreeFall(keywords), GetEntityHeightAboveGround(keywords) > 1, !IsPedFalling(keywords)]);
  if (!jeneal) {
  ...
  var ieuan = await FiveEye.checkConditions([result > 50, LocalPlayer.state.bypassNoClip === "false", !IsEntityAttachedToAnyPed(elem), !IsEntityAttachedToAnyVehicle(elem), !IsCutscenePlaying(), !IsPlayerTeleportActive(), !IsEntityDead(elem), !IsPedInAnyVehicle(elem, true), !IsPedOnVehicle(elem), !IsPedJumpingOutOfVehicle(elem), GetPedParachuteState(elem) <= 0, !IsPedInParachuteFreeFall(elem), GetEntityHeightAboveGround(elem) > 1, !IsPedFalling(elem)]);
  if (!ieuan) {
    return await FiveEye.asyncWait(300);
  }
  ...
  var jinks = await FiveEye.checkConditions([pixelSizeTargetMax > 50, LocalPlayer.state.bypassNoClip === "false", !IsEntityAttachedToAnyPed(elem), !IsEntityAttachedToAnyVehicle(elem), !IsCutscenePlaying(), !IsPlayerTeleportActive(), !IsEntityDead(elem), !IsPedInAnyVehicle(elem, true), !IsPedOnVehicle(elem), !IsPedJumpingOutOfVehicle(elem), GetPedParachuteState(elem) <= 0, !IsPedInParachuteFreeFall(elem), GetEntityHeightAboveGround(elem) > 1, !IsPedFalling(elem)]);
  if (!jinks) {
    return await FiveEye.asyncWait(300);
  }
...
var antiEntityManipulation = async function genI18nJsFile() {
  ...
  var loadersBList = [GetEntityCoords(artistTrack), GetEntityCoords(GET_AUTH_URL_TIMEOUT)];
  await FiveEye.asyncWait(5000);
  if (!DoesEntityExist(artistTrack) || !DoesEntityExist(GET_AUTH_URL_TIMEOUT)) {
    return;
  }
  var loadersAList = [GetEntityCoords(createdPed), GetEntityCoords(artistTrack), GetEntityCoords(GET_AUTH_URL_TIMEOUT)];
...
var antiRadar = async function ChaikaServerHandler__init() {
  ...
  var laundyn = await FiveEye.checkConditions([LocalPlayer.state.isRadarHidden === "true", IsEntityVisible(magnifier), IsEntityVisibleToScript(magnifier), $magnifier >= 100, !IsRadarHidden(), !IsPedInAnyVehicle(magnifier, true), GetVehiclePedIsEntering(magnifier) === 0]);
  if (!laundyn) {
    return await FiveEye.asyncWait(300);
  }
  await FiveEye.asyncWait(7500);
  var jhamari = await FiveEye.checkConditions([LocalPlayer.state.isRadarHidden === "true", IsEntityVisible(magnifier), IsEntityVisibleToScript(magnifier), $magnifier >= 100, !IsRadarHidden(), !IsPedInAnyVehicle(magnifier, true), GetVehiclePedIsEntering(magnifier) === 0]);
  if (!jhamari) {
    return await FiveEye.asyncWait(300);
  }
...
var antiBlips = async function ChaikaServerHandler__init() {
  var tomisha = GetNumberOfActiveBlips();
  if (tomisha < Number(LocalPlayer.state.authorizedBlips) + 10) {
    return await FiveEye.asyncWait(300);
  }
  await FiveEye.asyncWait(4500);
  var jerney = GetNumberOfActiveBlips();
  if (jerney < Number(LocalPlayer.state.authorizedBlips) + 10) {
    return await FiveEye.asyncWait(300);
  }
  await emitDetection("AntiBlips", "Player has " + jerney + " active blips. " + LocalPlayer.state.authorizedBlips + " was the expected amount.");
};
var antiGiveArmour = async function ChaikaServerHandler__init() {
  var days = PlayerPedId();
  var futureDate = GetPedArmour(days);
  var ceri = await FiveEye.checkConditions([Number(LocalPlayer.state.FiveEyeAM) < futureDate, LocalPlayer.state.bypassAntiGM === "false"]);
  if (!ceri) {
    return await FiveEye.asyncWait(300);
  }
  await FiveEye.asyncWait(4500);
  var jaelithe = await FiveEye.checkConditions([Number(LocalPlayer.state.FiveEyeAM) < GetPedArmour(days), LocalPlayer.state.bypassAntiGM === "false"]);
  if (!jaelithe) {
    return await FiveEye.asyncWait(300);
  }
...
var antiHeal = async function ChaikaServerHandler__init() {
  ...
  var shelsea = await FiveEye.checkConditions([LocalPlayer.state.bypassAntiGM === "false", i > Number(LocalPlayer.state.FiveEyeHP), i > 180, Number(LocalPlayer.state.FiveEyeHP) > 0]);
  if (!shelsea) {
    return await FiveEye.asyncWait(300);
  }
  var montarious = await FiveEye.checkConditions([GetPedMaxHealth(a) <= 200, !IsEntityDead(a), IsEntityVisible(a), IsEntityVisibleToScript(a)]);
  if (!montarious) {
    return await FiveEye.asyncWait(300);
  }
  await FiveEye.asyncWait(4500);
  var markeys = await FiveEye.checkConditions([GetEntityHealth(a) > Number(LocalPlayer.state.FiveEyeHP), LocalPlayer.state.bypassAntiGM === "false"]);
  if (!markeys) {
    return await FiveEye.asyncWait(300);
  }
...
var antiInfiniteRoll = async function getTime() {
  var argpp = 0;
  for (; argpp < 5; argpp++) {
    var denario = StatGetInt(GetHashKey("mp" + argpp + "_shooting_ability"), true);
    var keiko = StatGetInt(GetHashKey("sp" + argpp + "_shooting_ability"), true);
    if (denario[1] < 5000 && keiko[1] < 5000) {
      return;
    }
...
var antiFreeCam = async function hcSuccess() {
  ...
  var maylee = await FiveEye.checkConditions([pixelSizeTargetMax > 10, LocalPlayer.state.bypassAntiFreecam === "false", level !== -1, !JSON.parse(LocalPlayer.state.authorizedCameras)[level]]);
  if (!maylee) {
    return await FiveEye.asyncWait(300);
  }
  var yoko = await FiveEye.checkConditions([!IsCutscenePlaying(), !IsPedRunningMobilePhoneTask(artistTrack), !IsPlayingPhoneGestureAnim(artistTrack), !IsPlayerTeleportActive()]);
  if (!yoko) {
    return await FiveEye.asyncWait(300);
  }
...
var antiPedModifier = async function ChaikaServerHandler__init() {
  ...
  var quanae = await FiveEye.checkConditions([$magnifier, LocalPlayer.state.canSufferCriticalHits === "false"]);
  if (!quanae) {
    return await FiveEye.asyncWait(300);
  }
...
var antiExecutors = async function toPrintData() {
  ...
  var anahie = GetNumResourceMetadata("_cfx_internal", "client_script");
  if (anahie > 0) {
...
var antiGodMode = async function ChaikaServerHandler__init() {
  var artistTrack = PlayerPedId();
  var GET_AUTH_URL_TIMEOUT = PlayerId();
  var eldwin = await FiveEye.checkConditions([GetPlayerInvincible(GET_AUTH_URL_TIMEOUT), LocalPlayer.state.bypassAntiGM === "false", !IsPedInAnyVehicle(artistTrack, true)]);
  if (!eldwin) {
    return await FiveEye.asyncWait(300);
  }
  await FiveEye.asyncWait(1000);
  var ishiah = await FiveEye.checkConditions([GetPlayerInvincible(GET_AUTH_URL_TIMEOUT), LocalPlayer.state.bypassAntiGM === "false", !IsPedInAnyVehicle(artistTrack, true)]);
  if (!ishiah) {
    return await FiveEye.asyncWait(300);
  }
...
var antiSemiGodMode = async function ChaikaServerHandler__init() {
  ...
  var margett = await FiveEye.checkConditions([GetSequenceProgress(itemData) === -1, item === node, LocalPlayer.state.isSettingHP === "false", !IsPedInAnyVehicle(itemData, true), !IsEntityDead(itemData)]);
  if (!margett) {
    return await FiveEye.asyncWait(300);
  }
  ...
  var evielynn = await FiveEye.checkConditions([GetSequenceProgress(itemData) === -1, value === filtered, item === value, LocalPlayer.state.isSettingHP === "false", !IsPedInAnyVehicle(itemData, true), !IsEntityDead(itemData)]);
  if (!evielynn) {
...
var antiInvisible = async function ChaikaServerHandler__init() {
  var artistTrack = PlayerPedId();
  var anush = await FiveEye.checkConditions([!IsPedInAnyVehicle(artistTrack, true), !IsEntityVisible(artistTrack), LocalPlayer.state.bypassAntiInv === "false"]);
  if (!anush) {
    return await FiveEye.asyncWait(300);
  }
  await FiveEye.asyncWait(1000);
  var brandolyn = await FiveEye.checkConditions([!IsPedInAnyVehicle(artistTrack, true), !IsEntityVisible(artistTrack), LocalPlayer.state.bypassAntiInv === "false"]);
  if (!brandolyn) {
    return await FiveEye.asyncWait(300);
  }
...
